...
def TF_FullConnForwardPropagation_BatchNorm_Train_test(s, n, X, Activator, Train):
    l_max = len(n)
    A = 0
    ...
    for i in range(l_max):
        ...
        mean_mv = tf.Variable(tf.zeros([n[i],1]), dtype=tf.float32, trainable=False)
        var_mv = tf.Variable(tf.ones([n[i],1]), dtype=tf.float32, trainable=False)
        
        if Train == True:
            mean = fc_mean
            var = fc_var
            mean_mv, var_mv = mean_var_with_update()
        else :
            mean = mean_mv
            var = var_mv
        Z_norm = tf.nn.batch_normalization(Z, mean, var, shift, scale, epsilon)                                                
